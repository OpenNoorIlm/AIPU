...
Supports: --flash (default), --idcode, --verify, --verify-only, --watch
...
# ── Verify (must be called BEFORE IR_PROG_INACTIVE) ──────────────────────────

def verify_sram(data: bytes) -> bool:
    """
    Read back first 256 bytes of config SRAM and compare against bitstream.
    MUST be called while still in config mode (before deactivating).
    TAP should be in Run-Test/Idle when called.
    """
    sample = min(256, len(data))
    print(f"Verifying first {sample} bytes from SRAM...")

    shift_ir(IR_INIT_ADDR)
    write_dr(0x00, 8)

    shift_ir(IR_READ_SRAM)
    clock(1, 0); clock(0, 0); clock(0, 0)  # → Shift-DR
    ser.flush()
    ...
    for i in range(sample * 8):
        ...
        if i % 8 == 0:
            readback.append(0)
            pct = len(readback) * 100 // sample
            print(f"\r  Reading {len(readback)}/{sample} bytes ({pct}%)", end='', flush=True)
        readback[-1] |= bit << (i % 8)
    print()

    clock(1, 0); clock(1, 0); clock(0, 0)  # → Update-DR, Idle
    ser.flush()

    mismatches = 0
    for i, (got, exp) in enumerate(zip(readback, data[:sample])):
        if got != exp:
            print(f"  MISMATCH byte {i}: got 0x{got:02X} expected 0x{exp:02X}")
            mismatches += 1
            if mismatches >= 5:
    ...
    if mismatches == 0:
        print(f"✅ Verify OK — first {sample} bytes match")
        return True
    else:
        print(f"❌ Verify FAILED — {mismatches} mismatches")
        print("   This usually means a JTAG timing issue, not a corrupt bitstream.")
        print("   Try reflashing — the FPGA is likely running fine regardless.")
        return False

def verify_only(path: str):
    """
    --verify-only: re-program SRAM just to read it back, then deactivate.
    This is the only way to verify without a prior flash in the same session
    (SRAM is volatile — it must be re-written to be readable via JTAG).
    """
    ...
    print("Re-programming SRAM for verify...")
    ...
    if ok:
        print("✅ Verify-only complete — AIPU running!")
    else:
        print("⚠️  Verify failed but AIPU may still be running — check UART output.")

# ── Flash ─────────────────────────────────────────────────────────────────────

def flash(path, do_verify=False):
    if not os.path.exists(path):
    ...
    # ── Verify BEFORE deactivating (SRAM only readable while in config mode) ──
    if do_verify:
        verify_sram(data)
...
def main():
    ...
    parser.add_argument("--verify",      action="store_true", help="Verify SRAM after flash (before deactivate)")
    parser.add_argument("--verify-only", action="store_true", help="Re-flash + verify only (no watch, confirms bitstream integrity)")
    ...
    flash(args.fs, do_verify=args.verify)
